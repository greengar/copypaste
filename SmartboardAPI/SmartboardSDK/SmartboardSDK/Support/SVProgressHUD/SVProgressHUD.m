...
- (void)showImmediatelyWithType:(SVProgressType)type withMessage:(NSString*)message error:(BOOL)error;
- (void)showImmediatelyWithType:(SVProgressType)type withMessage:(NSString*)message error:(BOOL)error afterDelay:(NSTimeInterval)seconds;
- (void)setStatus:(NSString*)string;

- (void)dismiss;
- (void)dismissWithStatus:(NSString*)string error:(BOOL)error;
- (void)dismissWithStatus:(NSString*)string error:(BOOL)error afterDelay:(NSTimeInterval)seconds;
- (void)dismissWithType:(SVProgressType)type withMessage:(NSString*)message error:(BOOL)error;
- (void)dismissWithType:(SVProgressType)type withMessage:(NSString*)message error:(BOOL)error afterDelay:(NSTimeInterval)seconds;
...
+ (void)showImmediatelyWithType:(SVProgressType)type withMessage:(NSString *)message {
    [SVProgressHUD showImmediatelyWithType:type withMessage:message afterDelay:0.1];
}

+ (void)showImmediatelyWithType:(SVProgressType)type withMessage:(NSString *)message afterDelay:(NSTimeInterval)seconds {
    [SVProgressHUD showInView:nil status:nil networkIndicator:NO posY:-1 maskType:SVProgressHUDMaskTypeNone];
    [[SVProgressHUD sharedView] showImmediatelyWithType:type withMessage:message error:NO afterDelay:seconds];
}
...
+ (void)dismissWithType:(SVProgressType)type withMessage:(NSString *)message {
    [[SVProgressHUD sharedView] dismissWithType:type withMessage:message error:NO];
}

+ (void)dismissWithType:(SVProgressType)type withMessage:(NSString *)message afterDelay:(NSTimeInterval)seconds {
    [[SVProgressHUD sharedView] dismissWithType:type withMessage:message error:NO afterDelay:seconds];
}
...
- (void)setStatus:(NSString *)string {
    ...
    if (hudWidth >= [UIApplication sharedApplication].keyWindow.frame.size.width) {
        hudWidth = [UIApplication sharedApplication].keyWindow.frame.size.width - 10;
    }
    
    int numberOfLine = stringWidth / hudWidth + 1;
	
    _hudView.bounds = CGRectMake(0, 0, hudWidth, 85 + 22 * numberOfLine);
    self.stringLabel.frame = CGRectMake(0, 66, CGRectGetWidth(_hudView.bounds), 20 + 20 * numberOfLine);
...
- (void)showInView:(UIView*)view status:(NSString*)string networkIndicator:(BOOL)show posY:(CGFloat)posY maskType:(SVProgressHUDMaskType)hudMaskType {
    ...
    if(self.maskType != SVProgressHUDMaskTypeNone)
        ...
        self.userInteractionEnabled = NO; // Hector: We should block UI when showing Progress View
...
- (void)showImmediatelyWithType:(SVProgressType)type withMessage:(NSString *)message error:(BOOL)error {
    [self showImmediatelyWithType:type withMessage:message error:NO afterDelay:0.0];
}

- (void)showImmediatelyWithType:(SVProgressType)type withMessage:(NSString *)message error:(BOOL)error afterDelay:(NSTimeInterval)seconds {
    [UIApplication sharedApplication].networkActivityIndicatorVisible = YES;
    [self dismissWithType:type withMessage:message error:error afterDelay:0.9];
}
...
- (void)dismissWithType:(SVProgressType)type withMessage:(NSString *)message error:(BOOL)error afterDelay:(NSTimeInterval)seconds {
    switch (type) {
        case SVProgressHUDTypeLove:
            self.imageView.image = [UIImage imageNamed:@"love"];
            break;
        case SVProgressHUDTypeComment:
            self.imageView.image = [UIImage imageNamed:@"comment"];
            break;
        case SVProgressHUDTypeFriend:
	...
	fadeOutTimer = [[NSTimer scheduledTimerWithTimeInterval:seconds target:self selector:@selector(dismissImmediately) userInfo:nil repeats:NO] retain];
}
...
- (UILabel *)stringLabel {
    ...
    if (stringLabel == nil) {
        ...
        stringLabel.lineBreakMode = UILineBreakModeWordWrap;
