...
- (void)layoutSubviews
{
    CGRect frame = CGRectMake(5, 5, 75, 75);// self.imagePickerController.itemRect;
    CGFloat leftMargin = 5;
    
	for (AGIPCGridItem *gridItem in self.items) {
		[gridItem setFrame:frame];
        UITapGestureRecognizer *selectionGestureRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:gridItem
                                                                                                     action:@selector(tap)];
